...
* Native FST hash computation now sorts arcs by input label and hashes a compact binary structural representation, improving hashing performance; existing native FST cache filenames may change.
* **Breaking: `WFST.get_fst_text()` no longer accepts an `fst_cache` argument.** Standalone FST text generation is now cache-independent.
* `WFST.compute_hash(seed, eps2disambig=False)` was added, giving both `WFST` and `NativeWFST` dependency-seeded hashing APIs.
...
* **Breaking: native build selection now uses `KALDI_REVISION` instead of `KALDI_BRANCH`.** CMake reads the environment variable or `kaldi-native-revision.txt` and stops configuration unless the value is a full lowercase 40-character commit hash. `building/checkout-kaldi.sh` replaces shallow branch/tag cloning with checkout of that exact revision.
...
* **Breaking: `max_num_rules` is no longer an explicit nnet3 decoder constructor parameter.** Supply it as a decoder configuration keyword instead. Callers using its former positional slot must migrate to named arguments; otherwise the value silently binds to `save_adaptation_state`.
* Active nnet3 decoder configuration now defines shared `max_num_rules` and `max_num_exported_rules` keys, defaulting to 10000 and 1000 respectively (`max_num_rules` previously defaulted to 9999). `Compiler` supplies both values from its exported/non-exported rule-ID allocator.
...
* Fixed symbol-table lookups accepting prefixes instead of requiring complete symbol matches.
...
* Test tooling now requires `psutil>=5.9` for process and resource metrics, and the new project-level `TODO.md` tracks known limitations and planned work.
...
* Documented platform-specific wheels as the only supported distribution because a usable installation must contain the matching native Kaldi library; source distributions (`sdist`) remain intentionally unsupported. Also documented `KALDIAG_BUILD_SKIP_NATIVE=1` for source-tree wheel builds whose matching native artifacts have already been staged.
