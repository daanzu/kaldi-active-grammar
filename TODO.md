...
- Test the CLI path that still uses file-based AGF compilation:
  `python -m kaldi_active_grammar compile_agf_dictation_graph`.

### Grammar lifecycle memory coverage
...
### Python/native release coordination
...
### macOS ARM stress-test memory usage
...
#### Naive decode FST activity for nested rules
...
### Grammar lifecycle and representation
...
#### Grammar add/remove decode-FST teardown
...
### Activity updates and cache invalidation

#### Compose-cache invalidation predicate
...
- **Do not** instead relax the predicate so cache absence is never evidence of
  ...
  rebuilding the graph (see the teardown item above).
...
### Decode performance and memory

#### Dictation call-site duplication
...
#### Replace-state caching and residual lookahead cost
...
### Accuracy and framework positioning
